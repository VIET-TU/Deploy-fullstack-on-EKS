...
#****** Nếu như trong quá trình tạo bị lỗi không xóa được thì vào CloudFormation Stack để xóa Stack đi
...
## --managed yêu cầu tạo mặc định không có custom gì
## Việc tạo nhiều node group trong một cluster ==> phục vụ các mục đích khác nhau cho nhóm service, VD service nào cần cpu nhiều ta tạo node group gồm con mạnh hơn, VD service nào cần nhiều ram thì tạo node group
